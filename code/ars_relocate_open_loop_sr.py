...
class Worker(object):
    ...
    def rollout(self, shift = 0., rollout_length = None):
        ...
        return success_ctr > success_thresh, steps

    def do_rollouts(self, w_policy, num_rollouts = 1, shift = 1, evaluate = False, num_train_rollouts = 1):
        """ 
        ...
        for i in range(num_rollouts):
            ...
            if evaluate:
                ...
                success, r_steps = self.rollout(shift = 0., rollout_length = self.rollout_length)
                ...
                pos_success, pos_steps = np.zeros(num_train_rollouts), 0
                for j in range(num_train_rollouts):
                    pos_success[j], pos_rollout_steps = self.rollout(shift = shift)
                    pos_steps += pos_rollout_steps
                pos_success = pos_success.mean()

                # compute reward and number of timesteps used for negative pertubation rollout
                self.policy.update_weights(w_policy - delta)
                neg_success, neg_steps = np.zeros(num_train_rollouts), 0
                for j in range(num_train_rollouts):
                    neg_success[j], neg_rollout_steps = self.rollout(shift = shift)
                    neg_steps += neg_rollout_steps
                neg_success = neg_success.mean()
        ...
        return {'deltas_idx': deltas_idx, "steps" : steps, "successes": successes}
...
class ARSLearner(object):
    ...
    def __init__(self, task_id='relocate',
        ...
        self.train_rollout_multiplier = params['num_training_rollouts']
    ...
    def aggregate_rollouts(self, num_rollouts = None, evaluate = False, train_rollout_multiplier = 1):
        """ 
        Aggregate update step from rollouts generated in parallel.

        Added parameter: train_rollout_multiplier - number of times to run each new policy to get success rate in training
        """
        ...
        rollout_ids_one = [worker.do_rollouts.remote(policy_id,
                                                 ...
                                                 shift = self.shift,
                                                 evaluate=evaluate,
                                                 num_train_rollouts = train_rollout_multiplier) for worker in self.workers]

        rollout_ids_two = [worker.do_rollouts.remote(policy_id,
                                                 num_rollouts = 1,
                                                 shift = self.shift,
                                                 evaluate=evaluate,
                                                 num_train_rollouts = train_rollout_multiplier) for worker in self.workers[:(num_deltas % self.num_workers)]]

        # gather results 
        results_one = ray.get(rollout_ids_one)
        results_two = ray.get(rollout_ids_two)

        deltas_idx, success_rates = [], [] 
        ...
        for result in results_one:
            ...
            success_rates += result['successes']
        ...
        successes = np.array(success_rates)
        t2 = time.time()
        ...
        max_successes = np.max(successes, axis = 1)
        if self.deltas_used > self.num_deltas:
            self.deltas_used = self.num_deltas
            
        idx = np.arange(max_successes.size)[max_successes >= np.percentile(max_successes, 100*(1 - (self.deltas_used / self.num_deltas)))]
        deltas_idx = deltas_idx[idx]
        successes = successes[idx,:]
        
        
        # shouldn't need this since successes are strictly in the [0, 1] range
        # normalize rewards by their standard deviation
        # successes /= np.std(successes)

        t1 = time.time()
        # aggregate rollouts to form g_hat, the gradient used to compute SGD step
        g_hat, count = utils.batched_weighted_sum(successes[:,0] - successes[:,1],
    ...
    def train_step(self):
        ...
        g_hat, successes = self.aggregate_rollouts(train_rollout_multiplier = self.train_rollout_multiplier)                  
    ...
    def train(self, num_iter, num_eval_rollouts = 100):
        ...
        train_sr = np.zeros((num_iter, self.num_deltas * 2))
        eval_sr = np.zeros((num_iter // 10, num_eval_rollouts))

        best_eval_policy_weights = self.w_policy #initial weights
        best_eval_policy_sr = float('-inf')
        best_filter_mean, best_filter_std = 0, 1

        start = time.time()
        for i in tqdm(range(num_iter)):
            t1 = time.time()
            step_success_rates = self.train_step()
            t2 = time.time()

            #mean_reward = step_rewards.mean()
            train_sr[i, :] = step_success_rates.flatten()
            ...
            if ((i + 1) % 10 == 0):
                success_rates = self.aggregate_rollouts(num_rollouts = num_eval_rollouts, evaluate = True)
                w = ray.get(self.workers[0].get_weights_plus_stats.remote())
                # np.save(self.logdir + "/koopman_policy.npy", w)

                eval_sr[i // 10, :] = success_rates.flatten()
                if success_rates.mean() > best_eval_policy_sr:
                    best_eval_policy_sr = success_rates.mean()
                ...
                logz.log_tabular("Task Success Rate", success_rates.mean())
        ...
        #save best weights
        print(f'Best eval policy success rate: {best_eval_policy_sr}')
        ...
        np.save(os.path.join(self.logdir, 'training_sr.npy'), train_sr)
        np.save(os.path.join(self.logdir, 'eval_sr.npy'), eval_sr)
...
def run_ars(params):
    ...
    train_sr, eval_sr = ARS.train(params['n_iter'])
    graph_results.graph_training_and_eval_rewards(train_sr, eval_sr, logdir, False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_directions', '-nd', type=int, default=80) #directions explored - results in 2*d actual policies
    parser.add_argument('--deltas_used', '-du', type=int, default=20) #directions kept for gradient update
    parser.add_argument('--step_size', '-s', type=float, default=0.2)#0.02, alpha in the paper
    ...
    parser.add_argument('--num_training_rollouts', type = int, default = 10)
